 ...
 * 
 * getTable(sqlName) : 根据数据库表名,得到table对象 getAllTable() : 搜索数据库的所有表,并得到table对象列表
 * 
 * @author verne
 */
public class TableFactory {
    ...
    private Table createTable(Connection conn, ResultSet rs) throws SQLException {
        ...
        try {
            ...
            DatabaseMetaData dbMetaData = conn.getMetaData();
            indexRs = dbMetaData.getIndexInfo(getCatalog(), getSchema(), table.getTableName(),
                    false, true);
            while (indexRs.next()) {
            ...
            for (Column column : columns) {
                ...
                String searchColumn = com.kmob.generator.util.Config
                        .getStr("template.table." + table.getTableName() + ".searchColumn");
    ...
    private List<Column> getTableColumns(Connection conn, Table table, List<String> indices,
            Map<String, String> uniques) throws SQLException {
        // get the columns
        List<Column> columns = new LinkedList<Column>();
        ResultSet columnRs = conn.getMetaData().getColumns(getCatalog(), getSchema(),
    ...
    private List<String> getTablePrimaryKeys(Table table, Connection conn) throws SQLException {
        ...
        ResultSet primaryKeyRs =
                conn.getMetaData().getPrimaryKeys(getCatalog(), getSchema(), table.getTableName());
